...
export const CertificateCategoryManager = () => {
  const { certificateCategories, addCertificateCategory, updateCertificateCategory, deleteCertificateCategory } = useAdminStore();
  ...
  const onSubmit = async (data: CategoryFormData) => {
    ...
    try {
      if (editingId) {
        await updateCertificateCategory(editingId, { name: data.name });
        toast.success('Kategori berhasil diperbarui');
        setEditingId(null);
      } else {
        await addCertificateCategory({ name: data.name, slug: '' }); // Slug handled by backend
  ...
  const handleDelete = async (id: number) => {
    if (confirm('Apakah Anda yakin ingin menghapus kategori ini? Sertifikat di dalamnya mungkin akan kehilangan kategori.')) {
      try {
        await deleteCertificateCategory(id);
  ...
  return (
    <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
      ...
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          <DialogTitle>Kelola Kategori Sertifikat</DialogTitle>
          <DialogDescription>
            Tambah, ubah, atau hapus kategori untuk sertifikat Anda.
          </DialogDescription>
        </DialogHeader>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="flex gap-2 items-end">
            <div className="grid w-full gap-2">
              ...
              <Input
                ...
                placeholder="Contoh: Frontend, Backend, Workshop"
              />
